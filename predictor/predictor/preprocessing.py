...
# 中央競馬（JRA）10場の固定リスト。地方競馬向けの別モデル（NAR_VENUES）分離に伴い、
# 中央競馬モデルの学習・予測データから地方競馬を除外するために使う。NAR側の場名を
# 列挙して除外するより、変わらないJRAの場名をallowlistにする方が、今後DBに新しい
# 地方競馬場が増えても自動的に除外されて頑健。
JRA_VENUES: list[str] = [
