...
from repository.models import TrainerProfile
from .base import BaseParser

logger = logging.getLogger(__name__)

# db_prof_table のキーと TrainerProfile キーの対応
_PROF_KEY_MAP = {
    "所属": "所属",
    "生年月日": "生年月日",
    "初免許年": "初免許年",
...
class TrainerParser(BaseParser):
    """調教師プロフィールページ (/trainer/profile/{trainer_id}/) のHTMLパーサー."""

    def parse(self, html: str) -> TrainerProfile:
        soup = self.parse_html(html)
        profile: TrainerProfile = {}

        # 調教師名: <h1> または <div class="horse_title"> 内の <h1>
        title_div = soup.find("div", class_="horse_title")
        if title_div:
            h1 = title_div.find("h1")
            if h1:
                profile["調教師名"] = h1.get_text(strip=True)
        else:
            h1 = soup.find("h1")
            if h1:
                profile["調教師名"] = h1.get_text(strip=True)
