...
- [x] レース一覧ページの総ページ数判定バグを発見・修正 — 2026-08-11、地方競馬パイロット準備中に発覚。
      `parse_total_pages` が常に1を返す不具合で、golem稼働開始（2026-04-26）以降、中央・地方問わず
...
- [ ] キャッチアップ（2026-08-11夜、golemでバックグラウンド開始、`scrape_incremental 2026 4`〜現在）の
      完了確認。完了後 `check_missing.py` を再実行し、月別件数がnetkeiba実測値に近づいたか確認する
- [x] venue絞り込みの地方競馬遡及バックフィル機能（`backfill_regional`）を実装 — 2026-08-11、
      大井150件のパイロットをgolemで実行し成功（0エラー）。ただし直近数ヶ月の未取得分の消化が
      大半で、本当に古い年代（2015年以前等）の検証は別途必要
