...
> **`race.betting`（軸）は未検証の参考表示であり、検証済み戦略ではない。** 上記の単勝・馬連・ワイド・三連複の基準は、どの人気帯についても walk-forward pooled bootstrap CI で回収率100%超えが示されたことは一度もない（`experiments.md` の複数箇所で単勝top1戦略は65〜80%止まりと繰り返し確認済み。2026-08-02 の7番人気以下タイ再検証でも馬連・三連複・ワイドいずれもCI上限が100%未達）。実際にベット判断の根拠として使ってよいのは、bootstrap CI で個別に検証されている下記「戦略別（堅実・大穴）」の2つだけであり、`race.betting` は「モデルが現在どの馬を上位視しているか」を見るための参考情報に留める。

> **軸と「大穴」戦略の重複防止（2026-08-02）**: レース内 `win_prob` 最大馬（top1）が人気帯「7番人気以下」に該当する場合、単勝・馬連・ワイド・三連複の推奨（複勝を除く）は出力しない。理由: 「大穴」戦略（下記）は top1 が7番人気以下のときに発火する設計のため、素の推奨をそのまま出すと軸側の推奨と「大穴」戦略が同一馬に収束し、2つの戦略が実質1つになってしまう（2026-08-02 クイーンS で実際に発生。詳細は [experiments.md](./experiments.md) 参照）。複勝は `place_prob` 上位3頭という別基準のため対象外。
